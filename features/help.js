module.exports.help = function help(Telegraf, ctx, bot) {
	const helpMenu = Telegraf.Extra.markdown().markup((m) =>
		m.inlineKeyboard(
			[
	...
	bot.action('conversation', (ctx) =>
		ctx.answerCbQuery('Hi', false).then(() => {
			ctx.replyWithMarkdown(
				'If you message Hi/Hello/Hey the bot will reply you with Hi and your user name'
			)
		})
	)

	bot.action('github', (ctx) =>
		ctx.answerCbQuery('GitHub', false).then(() => {
			ctx.replyWithMarkdown(
	...
	bot.action('dice', (ctx) =>
		ctx.answerCbQuery('Roll Dice', false).then(() => {
			ctx.replyWithMarkdown(
	...
	bot.action('poll', (ctx) =>
		ctx.answerCbQuery('Poll', false).then(() => {
			ctx.replyWithMarkdown(
	...
	bot.action('pin', (ctx) =>
		ctx.answerCbQuery('Pin', false).then(() => {
			ctx.replyWithMarkdown(
